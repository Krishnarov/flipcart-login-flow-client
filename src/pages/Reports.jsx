...
function Reports() {
  ...
  const fetchAll = useCallback(async () => {
    try {
      const [jobsRes, slotsRes] = await Promise.all([
        fetch(
          `${import.meta.env.VITE_API_URL}/api/emails/files?page=1&limit=100&sortField=createdAt&sortOrder=desc`,
          { headers: { Authorization: `Bearer ${token()}` } },
        ),
        fetch(`${import.meta.env.VITE_API_URL}/api/emails/slots`, {
          headers: { Authorization: `Bearer ${token()}` },
        }),
      ]);
  ...
  useEffect(() => {
    const fetchGlobalStats = async () => {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/emails/global-report?${params}`,
        { headers: { Authorization: `Bearer ${token()}` } },
      );
  ...
  const setExp = (key, val) =>
    setExportingMap((prev) => ({ ...prev, [key]: val }));
  const isExp = (key) => exportingMap[key];

  const convertCookies = (cookies) => cookies.map(c => {
    const sameSiteMap = { 'None': 'no_restriction', 'Lax': 'lax', 'Strict': 'strict' };
    const isSession = !c.expires || c.expires === -1;
    const obj = {
      domain: c.domain,
      ...
      name: c.name,
      path: c.path,
      sameSite: sameSiteMap[c.sameSite] || null,
      secure: c.secure,
      session: isSession,
      storeId: null,
      value: c.value,
    };
    if (!isSession) obj.expirationDate = c.expires;
    return obj;
  });
  ...
  const globalExportCookiesZip = async () => {
    ...
    try {
      ...
      if (successEmails.length === 0) {
        addToast("No cookies found", "error");
        return;
      }
      ...
      for (const e of successEmails) {
        folder.file(`${e.email}.json`, JSON.stringify(convertCookies(e.cookies), null, 2));
      }
      ...
      addToast("Cookies export failed", "error");
    } finally {
      setGlobalExporting("");
    }
  };
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      ...
      if (globalStatus && globalStatus !== "all")
        params.set("status", globalStatus);
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/emails/delete-all-data?${params}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token()}` },
        },
  ...
  const exportJobCSV = async (job, statusFilter) => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/emails/file-details?jobId=${job._id}&page=1&limit=10000`,
        { headers: { Authorization: `Bearer ${token()}` } },
      );
      const data = await res.json();
      if (!data.success) throw new Error("Failed");
      ...
      if (filtered.length === 0) {
        addToast(`No ${statusFilter} emails`, "error");
        return;
      }
  ...
  const exportJobCookiesZip = async (job) => {
    ...
    try {
      ...
      const successEmails = data.data.filter(
        (e) => e.status === "success" && e.cookies?.length > 0,
      );
      if (successEmails.length === 0) {
        addToast("No cookies found", "error");
        return;
      ...
      const zip = new JSZip();
      const folderName = job.uploadedFile
        .replace(/\.[^.]+$/, "")
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `${folderName}_cookies.zip`;
      a.click();
      URL.revokeObjectURL(url);
      addToast(`${successEmails.length} JSON files exported!`, "success");
    } catch {
    ...
    <>
      ...
      <div className="purchase-container">
          ...
          <div className="report-export-grid">
            <div className="report-export-card">
              ...
              <div className="report-export-info">
                ...
                <div className="report-export-desc">
                  All success by slot filter
                </div>
              </div>
              <button
                className="report-export-btn success-export-btn"
                onClick={() => globalExportCSV("success")}
                disabled={!!globalExporting}
              >
                {globalExporting === "global_success" ? (
                  <Spinner />
                ) : (
                  <Download size={14} />
                )}{" "}
            ...
            <div className="report-export-card">
              ...
              <div className="report-export-info">
                ...
                <div className="report-export-desc">
                  All failed by slot filter
                </div>
              </div>
              <button
                className="report-export-btn failed-export-btn"
                onClick={() => globalExportCSV("failed")}
                disabled={!!globalExporting}
              >
                {globalExporting === "global_failed" ? (
                  <Spinner />
                ) : (
                  <Download size={14} />
                )}{" "}
            ...
            <div className="report-export-card">
              ...
              <button
                className="report-export-btn cookies-export-btn"
                onClick={globalExportCookiesZip}
                disabled={!!globalExporting}
              >
                {globalExporting === "global_cookies" ? (
                  <Spinner />
                ) : (
                  <Download size={14} />
                )}{" "}
        ...
        <div className="card" style={{ padding: "24px" }}>
          ...
          <p className="card-subtitle" style={{ marginBottom: "20px" }}>
            Export reports per individual job.
          </p>
                  ...
                  {expandedJob === job._id && (
                    <div className="report-export-grid">
                      <div className="report-export-card">
                        ...
                        <div className="report-export-info">
                          <div className="report-export-title">Success</div>
                          <div className="report-export-desc">
                            {job.successCount} emails
                          </div>
                        </div>
                        <button
                          className="report-export-btn success-export-btn"
                          onClick={() => exportJobCSV(job, "success")}
                          disabled={
                            job.successCount === 0 ||
                            isExp(`${job._id}_success`)
                          }
                        >
                          {isExp(`${job._id}_success`) ? (
                            <Spinner />
                      ...
                      <div className="report-export-card">
                        ...
                        <div className="report-export-info">
                          <div className="report-export-title">Failed</div>
                          <div className="report-export-desc">
                            {job.failedCount} emails
                          </div>
                        </div>
                        <button
                          className="report-export-btn failed-export-btn"
                          onClick={() => exportJobCSV(job, "failed")}
                          disabled={
                            job.failedCount === 0 || isExp(`${job._id}_failed`)
                          }
                        >
                          {isExp(`${job._id}_failed`) ? (
                            <Spinner />
                          ) : (
                            <Download size={13} />
                          )}{" "}
                      ...
                      <div className="report-export-card">
                        ...
                        <div className="report-export-info">
                          <div className="report-export-title">Cookies ZIP</div>
                          <div className="report-export-desc">
                            {job.successCount} JSON files
                          </div>
                        </div>
                        <button
                          className="report-export-btn cookies-export-btn"
                          onClick={() => exportJobCookiesZip(job)}
                          disabled={
                            job.successCount === 0 ||
                            isExp(`${job._id}_cookies`)
                          }
                        >
                          {isExp(`${job._id}_cookies`) ? (
                            <Spinner />
                      ...
                      <div className="report-export-card">
                        ...
                        <div className="report-export-info">
                          <div className="report-export-title">Full Report</div>
                          <div className="report-export-desc">
                            {job.totalEmails} emails
                          </div>
                        </div>
                        <button
                          className="report-export-btn full-export-btn"
                          onClick={() => exportJobCSV(job, "all")}
                          disabled={
                            job.totalEmails === 0 || isExp(`${job._id}_all`)
                          }
                        >
                          {isExp(`${job._id}_all`) ? (
                            <Spinner />
